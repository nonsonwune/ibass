...
@bp.route('/institution/comment/reply', methods=['POST'])
@login_required
def add_institution_comment_reply():
    data = request.get_json()
    
    if not data or not all(k in data for k in ('parent_id', 'content', 'institution_id')):
...
@bp.route('/institution/comment/<int:comment_id>/delete', methods=['POST'])
@login_required
def delete_institution_comment(comment_id):
    try:
        ...
        if current_user.id != comment.user_id and not current_user.is_admin:
            ...
            return redirect(url_for('university.institution_details', id=comment.institution_id))
        ...
        return redirect(url_for('university.institution_details', id=institution_id))
